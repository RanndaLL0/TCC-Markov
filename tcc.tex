...
\begin{enumerate}
    ...
    \item Os nós expressam o consenso de aprovação para o bloco usando o para criar o próximo bloco da cadeia, usando o hash do bloco aceito.
\end{enumerate}
...
é o desvio padrão amostral. Os símbolos $-1$, $0$ e $1$ representam os movimentos de queda significante, oscilações pouco expressivas em torno da média e movimentos de alta significância, é necessário ressaltar que o trabalho avaliará a sensibilidade dos modelos também a limiares mais estreitos, como $\frac{s}{4}$ e $\frac{s}{5}$, o objetivo é garantir que o estado $0$ represente exclusivamente o ruído branco da microestrutura do mercado, reduzindo o desbalanceamento das classes. A discretização será aplicada separadamente sobre dois conjuntos distintos obtidos através da série dos retornos logarítmicos, conjunto de treinamento e teste, de maneira similar a \citeonline{Nascimento2023}, $95\%$ da série será separado para o treinamento dos modelos de predição e $5\%$ será reservado para o teste.


\subsubsection{Granularidade e viabilidade estatística}

Conforme conclui \citeonline{NASCIMENTO}, o custo computacional de treinamento de modelos de Cadeia de Markov de ordens superiores é de $O(n^{m+1})$, onde $n$ representa a granularidade do alfabeto discreto e $m$ a ordem da cadeia de Markov. Para entendermos a magnitude da complexidade da solução, considere um alfabeto de três símbolos. Uma cadeia de ordem $1$ possui $3^2=9$ probabilidades de transição, já uma cadeia de ordem $4$ terá $3^5=243$, e uma cadeia de ordem 6, $3^7=2.187$. Caso o alfabeto seja expandido para cinco símbolos, a mesma ordem 6 exige $5^7=78.125$. O crescimento exponencial, como conclui \citeonline{NASCIMENTO}, traz limites computacionais que tornam o uso de cadeias de ordens superiores restrito à complexidade do alfabeto simbólico construído e à dimensão do modelo em si. O modelo proposto por \citeonline{RAFTERY1985}, reduz a complexidade para $O(mn^2+m)$, assim tendo uma complexidade linear. Para um modelo de cinco símbolos e cadeia de ordem $6$ teríamos $6\cdot5^2+6=156$, uma redução significativa de probabilidades de transição.
...
\begin{table}[htb]
    ...
    \small
        ...
        \toprule
        \textbf{Critério} & \textbf{Araújo e Barbosa (2024)} & \textbf{Nascimento } & \textbf{Trabalho 3} & \textbf{Este trabalho} \\
        \midrule
        Objetivo & Reconstruir e prever processos de criptoativos e detectar memória de longo alcance & O estudo utiliza modelos de Cadeias de Markov (da 1ª à 10ª ordem) e categorização de dados para prever os valores de retornos futuros das quatro principais criptomoedas: Bitcoin, Ethereum, Litecoin e Ripple & O artigo busca prever as mudanças no preço do Bitcoin avaliando diferentes algoritmos de aprendizado de máquina com base na estrutura dos dados e métodos estatísticos & Prever a direção do BTC e identificar a ordem em que a memória se torna estatisticamente relevante \\
        \addlinespace
        Ativos & Bitcoin, Ethereum e Ripple & Bitcoin, Ethereum, Ripple, Litecoin & Bitcoin & BTC \\
        \addlinespace
        Granularidade & Intradiária (1 hora) & 0.1, 0.01, 0.001 e 0.0001 & Diária e intradiária (5 minutos) & Intradiária (5 minutos) \\
        \addlinespace
        Modelagem & Cadeias de Markov de alta ordem (\textit{Less-than-k}) & Cadeia de Markov de Alta Ordem & Regressão logística, Análise Discriminante Linear, XGBoost, Análise Discriminante Quadrática, Máquinas de vetores de suporte e \textit{Long Short-Term Memory} & Cadeias de Markov de alta ordem (MTD/Raftery) e HMM \\
        \addlinespace
        Discretização & Alfabetos de 5 e 3 símbolos & Não é empregado o uso de um alfabeto & Classificação binária & Alfabeto de 3 símbolos (limiares $s/3$, $s/4$, $s/5$) \\
        \addlinespace
        Validação & Erro médio de predição \textit{vs.} modelo aleatório & Raiz do Erro Quadrático Médio, Erro Absoluto Médio, Erro Percentual Absoluto Médio & Acurácia, precisão, \textit{recall}, \textit{F1-score} & Acurácia, precisão, \textit{recall}, F1 e matriz de confusão \textit{vs.} \textit{baselines} (aleatório e persistência) \\
        \addlinespace
        Limitação & Explosão combinatória limita a ordem ($k \geq 4$) & Explosão no consumo de memória limita a análise até a décima ordem & Granularidade e número de dimensões & --- \\
        \bottomrule
        \end{tabularx}
    \par\vspace{2pt}
