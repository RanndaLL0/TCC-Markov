...
\subsubsection{Estratégia 1: Criptomoedas, Economia e Alta Frequência}
\begin{description}
...
\subsubsection{Estratégia 2: Predição com Processos Estocásticos, Estatística e Métricas de Avalição}
\begin{description}
...
(\textit{True Positive Accuracy - TPA}). Precisão é defina por:
...
A medida F1 efetivamente relaciona a proporção de Verdadeiros Positivos (\textit{True Positives}) à média aritmética das previsões positivas (\textit{Predicted Positives}) 
e dos casos reais positivos (\textit{Real Positives}), é descrita como: 
...
Apesar de inicialmente as métricas serem definidas para um modelo de classificação binária em seu trabalho \citeonline{POWERS2020}conclue que essas relações podem ser 
...
estatisticamente irrelevantes ou ruido (0). Por consequência, a Acurácia global e a Taxa de Erro, embora úteis para uma visão geral, 
...
\begin{itemize}
    ...
    aproximadamente 920 mil registros. Sera aplicado o descarte em possiveis pontos em que exista janelas faltantes
    ...
    ira ser adotado o descarte das observações no momento da falha. Dessa forma, o cálculo de retorno ocorrerá exclusivamente entre janelas de cinco 
